...
//+------------------------------------------------------------------+
// 1.49  votknul injuk "i-Regr" dlja opredelenija macroTrenda (dimaa)
// 1.48++ Obrabotka, vykinul lushnee iz koda(na moj vkus) testitsja v razy bystree, auto_lotdecimal, autoPjatiznak (dimaa)
// 1.48T Uluchshennye vhody (made by dimaa)
// 1.47Т Добавлен индикатор биржевого времени
// 1.47: Введён режим "Turbo": первое колено=1/3 пипстепа,второе =2/3 пипстепа для повышения прибыльности во флете
//       Добавлен режим "NewCycle" отмены выставления новых ордеров. Открытые серии продолжают работать
//       Отключен информационный индикатор (для ускорения тестирования и оптимизации), так же при работе часто
//       вызывал зависание терминала
//       Добавлена переменная PeriodRSI, позволяющая выбирать таймфрейм 
//       для RSI:1=M1, 2=M5, 3=M15, 4=M30, 5=H1, 6=H4, 7=D1, 8=W1, 9=MN1, 0=Period() 
// 1.46: добавлена нормализация цен при расчёте OrderSend, чем избавились от ошибок при расчёте ордеров
...
// 1.44: исправлен баг, присутствующий в версии 1.43 (не работал один из иланов при включеном ММ)
#property copyright "Night"
#property link      "http://alfonius@yandex.ru"

//===================================================================
//------------------Общие настройки--------------------------------//
//===================================================================

extern string t1 = "Общие настройки";
extern double Lots = 0.01;          // теперь можно и микролоты 0.01 при этом если стоит 0.1 то следующий лот в серии будет 0.16
extern double LotExponent = 1.95;   // умножение лотов в серии по експоненте для вывода в безубыток. первый лот 0.1, серия: 0.15, 0.26, 0.43 ...
int    lotdecimal;       // 2 - микролоты 0.01, 1 - мини лоты 0.1, 0 - нормальные лоты 1.0
extern double PipStep_ = 30.0;       // шаг колена
extern double TakeProfit_ = 10.0;    // тейк профит
double PipStep;      
double TakeProfit;   
extern bool   Turbo = true;         // Турбо-режим :)
extern bool   NewCycle = true;      // Режим отмены выставления новых ордеров
extern int    PeriodRSI = 2;        // 1=M1, 2=M5, 3=M15, 4=M30, 5=H1, 6=H4, 7=D1, 8=W1, 9=MN1, 0=текущий график
int PerRSI;                         // Период RSI
extern bool   MM =true;            // ММ - манименеджмент
...
double TrailStart_ = 13.0;           // трейлингстарт
double TrailStop_ = 3.0;             //трейлингстоп
double TrailStart;           // трейлингстарт
double TrailStop;             //трейлингстоп
double slip = 5.0;                  // проскальзывание
extern string _MaCd_ = "Parmetry_MACDa_dlja opredelenija microTrenda";
extern int    macdFast = 12;            // parameters for MACD
extern int    macdSlow = 26;            // parameters for MACD
extern int    macdsignalPeriod = 9;        // parameters for MACD 
extern int    MACDtimeFrame_ = 0;           // parameters for MACD// 1=M1, 2=M5, 3=M15, 4=M30, 5=H1, 6=H4, 7=D1, 8=W1, 9=MN1, 0=текущий график
int MACDtimeFrame;
extern string _tmp2_ = " --- i-Regr --- dlja opredelenija macroTrenda";
...
int kanal=0; // вариант работы с границами канал, при =2 не открывааем позиции за границами канала, =1 только за пределами одной границы, = 0 не учитываем границы только измененгие основной линии регресс
...
int invert=0; // если = 0 то по тренду, если =1 то работаем против глобального тренда, можно использовать с локом
...
if (Digits==5 || Digits == 3)x=10; else x=1;
TrailStart=TrailStart_*x;           // трейлингстарт
TrailStop=TrailStop_*x;             //трейлингстоп
PipStep=PipStep_*x;      
TakeProfit=TakeProfit_*x;   

if((MarketInfo(Symbol(), MODE_MINLOT))>0.09) lotdecimal =1; else lotdecimal=2;
if((MarketInfo(Symbol(), MODE_MINLOT))>0.9) lotdecimal =0;

   if (MACDtimeFrame_ == 1) MACDtimeFrame =PERIOD_M1; 
   if (MACDtimeFrame_ == 2) MACDtimeFrame =PERIOD_M5; 
   if (MACDtimeFrame_ == 3) MACDtimeFrame =PERIOD_M15; 
   if (MACDtimeFrame_ == 4) MACDtimeFrame =PERIOD_M30; 
   if (MACDtimeFrame_ == 5) MACDtimeFrame =PERIOD_H1; 
   if (MACDtimeFrame_ == 6) MACDtimeFrame =PERIOD_H4; 
   if (MACDtimeFrame_ == 7) MACDtimeFrame =PERIOD_D1; 
   if (MACDtimeFrame_ == 8) MACDtimeFrame =PERIOD_W1; 
   if (MACDtimeFrame_ == 9) MACDtimeFrame =PERIOD_MN1;
   if (MACDtimeFrame_ == 0) MACDtimeFrame =Period();
...
int start()
 {
    ...
    Comment("" 
         ...
         + "_______________________________");
            ...
            double Trigger0=iMACD(NULL,MACDtimeFrame,macdFast,macdSlow,macdsignalPeriod,PRICE_CLOSE,MODE_MAIN,0);
            double Trigger1=iMACD(NULL,MACDtimeFrame,macdFast,macdSlow,macdsignalPeriod,PRICE_CLOSE,MODE_MAIN,1);
            double Trigger2=iMACD(NULL,MACDtimeFrame,macdFast,macdSlow,macdsignalPeriod,PRICE_CLOSE,MODE_MAIN,2);
...
//=======================================================================//
  {double PrevCl_Hilo; //переменная Hilo
   ...
   if (total_Hilo > 0 && total_Hilo <= MaxTrades_Hilo) {
      ...
      if (LongTrade_Hilo && LastBuyPrice_Hilo - NormalizeDouble(Ask,Digits) >= PipStep_Hilo * Point) TradeNow_Hilo = TRUE;
      if (ShortTrade_Hilo && NormalizeDouble(Bid,Digits) - LastSellPrice_Hilo >= PipStep_Hilo * Point) TradeNow_Hilo = TRUE;
   }
   if (total_Hilo < 1) {
      ShortTrade_Hilo = FALSE;
      LongTrade_Hilo = FALSE;
...
//HHHHHHHH~~~~~~~~~~~~~ Индюк RSI ~~~~~~~~~~HHHHHHHHH~~~~~~~~~~~~~~~//       
            if (iRSI(NULL, PerRSI, 14, PRICE_CLOSE, 1) > 30.0 || (Trigger0<0) && (Trigger1<0)  &&  (Trigger2>0) && alert()==-1) {
...
//HHHHHHHH~~~~~~~~~~~~~ Индюк RSI ~~~~~~~~~HHHHHHHHHH~~~~~~~~~~~~~~~~~
            if (iRSI(NULL, PerRSI, 14, PRICE_CLOSE, 1) < 70.0 || (Trigger0 > 0) && (Trigger1 > 0)  &&  (Trigger2 < 0 )&&alert()==1) {
...
{Sleep(1000);RefreshRates();}                                                                      //..причём здесь добавлена проверка, котрая должна по идее исключить невыставление ТР
//===
NewOrdersPlaced_Hilo = FALSE;
}
}
...
//========================================================================//
   ...
   if (gi_304_15 > 0 && gi_304_15 <= MaxTrades_15) {
      ...
      if (gi_320_15 && gd_236_15 - NormalizeDouble(Ask,Digits) >= PipStep_15 * Point) gi_316_15 = TRUE;
      if (gi_324_15 && NormalizeDouble(Bid,Digits) - gd_244_15 >= PipStep_15 * Point) gi_316_15 = TRUE;
   }
   if (gi_304_15 < 1) {
      gi_324_15 = FALSE;
      gi_320_15 = FALSE;
   ...
   if (totals_15==0 || orders_15 < 1) {
      ...
      if (!gi_324_15 && !gi_320_15) {
         ...
         if (l_iclose_8 > l_iclose_16) { if (NewCycle || Trigger0<0 && Trigger1<0  &&  Trigger2>0&&alert()==-1) {//!!!
         ...
         }else { if (NewCycle || Trigger0 > 0 && Trigger1 > 0  &&  Trigger2 < 0 &&alert()==1 ) {//!!!
...
//   double l_iclose_16;
   ...
   if (gi_304_16 > 0 && gi_304_16 <= MaxTrades_16) {
      ...
      if (gi_320_16 && gd_236_16 - NormalizeDouble(Ask,Digits) >= PipStep_16 * Point) gi_316_16 = TRUE;
      if (gi_324_16 && NormalizeDouble(Bid,Digits) - gd_244_16 >= PipStep_16 * Point) gi_316_16 = TRUE;
   }
   if (gi_304_16 < 1) {
      gi_324_16 = FALSE;
      gi_320_16 = FALSE;
...
if (OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber_Hilo) {
if (OrderType() == OP_BUY) OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Bid,Digits), slip_Hilo, Blue);
if (OrderType() == OP_SELL) OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Ask,Digits), slip_Hilo, Red);
}
Sleep(1000);
}
}
...
stopcal_Hilo = NormalizeDouble(Bid,Digits) - stop_Hilo * Point;
...
if (profit_Hilo < pType_Hilo) continue;
stoptrade_Hilo = OrderStopLoss();
stopcal_Hilo = NormalizeDouble(Ask,Digits) + stop_Hilo * Point;
...
void CloseThisSymbolAll_15() {
   for (int l_pos_0_15 = OrdersTotal() - 1; l_pos_0_15 >= 0; l_pos_0_15--) {
      ...
      if (OrderSymbol() == Symbol()) {
         if (OrderSymbol() == Symbol() && OrderMagicNumber() == g_magic_176_15) {
            if (OrderType() == OP_BUY) OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Bid,Digits), slip_15, Blue);
            if (OrderType() == OP_SELL) OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Ask,Digits), slip_15, Red);
         }
         Sleep(1000);
      }
   }
         ...
         if (OrderSelect(l_pos_36_15, SELECT_BY_POS, MODE_TRADES)) {
            ...
            if (OrderSymbol() == Symbol() || OrderMagicNumber() == g_magic_176_15) {
               if (OrderType() == OP_BUY) {
                  ...
                  l_price_28_15 = NormalizeDouble(Bid,Digits) - ai_4_15 * Point;
               ...
               if (OrderType() == OP_SELL) {
                  ...
                  if (l_ticket_16_15 < ai_0_15) continue;
                  l_ord_stoploss_20_15 = OrderStopLoss();
                  l_price_28_15 = NormalizeDouble(Ask,Digits) + ai_4_15 * Point;
...
void CloseThisSymbolAll_16() {
   for (int l_pos_0_16 = OrdersTotal() - 1; l_pos_0_16 >= 0; l_pos_0_16--) {
      ...
      if (OrderSymbol() == Symbol()) {
         if (OrderSymbol() == Symbol() && OrderMagicNumber() == g_magic_176_16) {
            if (OrderType() == OP_BUY) OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Bid,Digits), slip_16, Blue);
            if (OrderType() == OP_SELL) OrderClose(OrderTicket(), OrderLots(), NormalizeDouble(Ask,Digits), slip_16, Red);
         }
         Sleep(1000);
      }
   }
         ...
         if (OrderSelect(l_pos_36, SELECT_BY_POS, MODE_TRADES)) {
            ...
            if (OrderSymbol() == Symbol() || OrderMagicNumber() == g_magic_176_16) {
               if (OrderType() == OP_BUY) {
                  ...
                  l_price_28_16 = NormalizeDouble(Bid,Digits) - ai_4_16 * Point;
               ...
               if (OrderType() == OP_SELL) {
                  ...
                  if (l_ticket_16_16 < ai_0_16) continue;
                  l_ord_stoploss_20_16 = OrderStopLoss();
                  l_price_28_16 = NormalizeDouble(Ask,Digits) + ai_4_16 * Point;
...
int alert()       
{
   ...
   else
   {
      ...
      if (kanal == 1)
      {
         if (ma>ma1 && NormalizeDouble(Bid,Digits) > m_d) x_r1 = 1;
         if (ma<ma1  && NormalizeDouble(Ask,Digits) < m_up) x_r1 = -1;
      }
      else if (kanal == 2)
      {
         if (ma>ma1 && NormalizeDouble(Bid,Digits) > m_d && NormalizeDouble(Ask,Digits) < m_up ) x_r1 = 1;
         if (ma<ma1  && NormalizeDouble(Ask,Digits) < m_up && NormalizeDouble(Bid,Digits) > m_d ) x_r1 = -1;
      }   
   }
   return(x_r1);
}
