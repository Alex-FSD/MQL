...
int start()
  {
   ...
   while (ObjectFind("NL")!=-1)
   {
      ...
      NL=NLchek();
      int p = (NL-SL)/Point;
      if (p>=0) txt1=StringConcatenate("Прибыль ордеров SELL = ",p," п ",DoubleToStr(p*SumLot*MarketInfo(Symbol(),MODE_TICKVALUE),2),AC);
      else txt1=StringConcatenate("Убыток ордеров SELL = ",p," п ",DoubleToStr(p*SumLot*MarketInfo(Symbol(),MODE_TICKVALUE),2),AC);
   ...
   if (SL==0) return(0);
   if(NormalizeDouble(SL-Ask,Digits)>StopLevel) txt2=StringConcatenate("SL = ",DoubleToStr(SL,Digits));   
   if(NormalizeDouble(Bid-SL,Digits)>StopLevel) txt2=StringConcatenate("TP = ",DoubleToStr(SL,Digits)); 
   
   int ret=MessageBox(StringConcatenate("Установить ",txt2," всем ордерам SELL ?"),"", MB_YESNO|MB_TOPMOST);
   if (ret==IDYES)
   {
      RefreshRates();
         ...
         if(OrderType()==OP_SELL)     
         {
            if(NormalizeDouble(SL-Ask,Digits)>StopLevel && SL!=OSL)     
            {
               if (OrderModify(Ticket,OOP,SL,NormalizeDouble(OrderTakeProfit(),Digits),0,White))
                  txt = StringConcatenate(txt,"\nВыставлен стоплосс ",DoubleToStr(SL,Digits)," SELL ордеру ",Ticket);
