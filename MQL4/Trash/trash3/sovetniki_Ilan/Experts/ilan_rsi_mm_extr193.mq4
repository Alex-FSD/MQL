...
// + добавлен параметр OpenNewCycle
// + ускорена работа советника, для ускорения работы (тестирования) берите еще и  минимально приемлимые параметры  SPer и SPermall а таймфрейм постарше
// отдельное спс Rigal в создании данного билда, "причесал" код !
...
extern double Risk = 2;          // первая сделка размером в % от депо, если = 0 то первая сделка открывается размером Lots
